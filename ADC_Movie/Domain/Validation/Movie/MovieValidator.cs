...
{
    public class MovieValidator : AbstractValidator<ADC_Movie.Domain.Entity.Movie>
    {
        public MovieValidator()
        {
            RuleFor(x => x.Senha).Equal(x => x.ConfirmaSenha)
                .WithMessage("{PropertyName} não é igual a {PropertyValue}");

            RuleFor(t => t.Title)
                .NotEmpty()
                .WithMessage("{PropertyName} é inválido");

            RuleFor(x => x.Cpf).ValidateCpf();
        }
